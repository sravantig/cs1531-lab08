    ...
    def car_search(self, name=None, model=None):
        ...
        for car in self._cars:
            ...
            if name is not None and name.lower() in c_name.lower():
                cars.append(car)
            if model is not None and model.lower() in c_model.lower():
                cars.append(car)
        return cars

    def make_booking(self, customer, period, car, location):
        if location.pickup is "": 
            raise BookingError(location, period,"Specify a valid start location")
        if location.dropoff is None:
            raise BookingError(location, period,"Specify a valid end location")
        """ if period is None: 
            raise BookingError(location, period,"Specify a valid start date")
        if period is None:
            raise BookingError(location, period,"Specify a valid end date")"""
        if period <= 0:
            raise BookingError(location, period,"Specify a valid booking period")
